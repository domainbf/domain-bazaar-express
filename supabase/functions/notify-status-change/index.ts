...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!record || !old_record) return json({ skipped: true, reason: "missing records" });
    if (record.status === old_record.status) return json({ skipped: true, reason: "status unchanged" });

    if (type === "offer") await handleOffer(supabase, record);
    else if (type === "kyc") await handleKyc(supabase, record);
...
async function handleOffer(supabase: any, record: any) {
  const buyerId = record.buyer_id;
  if (!buyerId) return;

  const { data: domain } = await supabase
    .from("domains").select("name").eq("id", record.domain_id).maybeSingle();
...
function buildOfferEmail(domain: string, record: any, statusLabel: string) {
  ...
  <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;color:#111">
    ...
    <a href="${origin()}/my-offers" style="display:inline-block;background:#000;color:#fff;text-decoration:none;padding:12px 22px;border-radius:8px;font-weight:500">查看我的报价</a>
...
function buildKycEmail(record: any, statusLabel: string) {
  ...
  <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;color:#111">
    ...
    <a href="${origin()}/seller/earnings" style="display:inline-block;background:#000;color:#fff;text-decoration:none;padding:12px 22px;border-radius:8px;font-weight:500">前往收入结算</a>
    <p style="color:#999;font-size:12px;margin-top:32px">此邮件为系统自动发送，请勿直接回复。</p>
  </div>`;
}
