...
export const SwipeableTabs = ({
  ...
  const handleDragEnd = useCallback((event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    ...
    if (Math.abs(velocity) > 500 || Math.abs(offset) > threshold) {
      if (velocity > 0 || offset > threshold) {
        if (activeIndex > 0) { setDirection(-1); onTabChange(tabs[activeIndex - 1].key); }
      } else {
        if (activeIndex < tabs.length - 1) { setDirection(1); onTabChange(tabs[activeIndex + 1].key); }
      }
    }
  ...
  return (
    <div className={`flex flex-col h-full ${className}`}>
      <div className="flex border-b border-border bg-background overflow-x-auto scrollbar-hide">
        {tabs.map((tab) => (
          <button
            key={tab.key}
      ...
      <div className="flex justify-center gap-1.5 py-2 bg-muted/50">
        {tabs.map((tab) => (
          <div
            key={tab.key}
            className={`h-1.5 rounded-full transition-all ${
              activeTab === tab.key ? 'w-4 bg-primary' : 'w-1.5 bg-muted-foreground/30'
            }`}
          />
        ))}
      </div>
      ...
      <div ref={containerRef} className="flex-1 overflow-hidden relative">
        <AnimatePresence mode="wait" custom={direction}>
          <motion.div
            ...
            onDragEnd={handleDragEnd} className="absolute inset-0 overflow-y-auto"
          >
