...
export const ResetPasswordConfirmForm = ({ tokenData }: ResetPasswordConfirmFormProps) => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (sessionError) throw new Error('重置链接已过期或无效，请重新请求密码重置');
  ...
  return (
    <Card className="w-full shadow-md border border-border">
      <CardHeader className={isSuccess ? "pb-2" : "pb-6"}>
        ...
        <CardDescription>
          {!isSuccess ? "请设置一个安全的新密码（设置成功后原密码将失效）" : "您的密码已成功更新"}
        </CardDescription>
      </CardHeader>
      
      <CardContent>
        {errorMessage && !isSuccess && (
          <div className="bg-destructive/10 border border-destructive/20 p-3 rounded-md flex items-start mb-4">
        ...
        {!isSuccess ? (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-primary/10 border border-primary/20 p-3 rounded-md flex items-start mb-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  required placeholder="输入您的新密码（至少6个字符）"
                  className="pr-10" minLength={6} disabled={isLoading}
                />
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  required placeholder="再次输入您的新密码"
                  className="pr-10" minLength={6} disabled={isLoading}
                />
              </div>
            </div>
          ...
          <div className="py-4">
            <div className="bg-primary/10 text-foreground p-4 rounded-md flex items-start">
              <CheckCircle className="w-5 h-5 mr-2 mt-0.5 flex-shrink-0 text-primary" />
              <div>
                <p className="font-medium">您的密码已更新！</p>
                <p className="mt-1 text-muted-foreground">正在跳转到用户中心...</p>
              </div>
            </div>
          </div>
        )}
      ...
      <CardFooter className={isSuccess ? "justify-center" : "justify-between"}>
        {!isSuccess ? (
          <>
            <Button variant="outline" onClick={() => navigate('/auth')} disabled={isLoading}>
              <ArrowLeft className="w-4 h-4 mr-2" /> 返回登录
            </Button>
            <Button onClick={handleSubmit} disabled={isLoading || !password || !confirmPassword}>
              {isLoading ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" /> 更新中...</>) : "确认修改密码"}
            </Button>
          </>
        ) : (
          <Button onClick={() => navigate('/user-center')} className="w-full">前往用户中心</Button>
        )}
      </CardFooter>
    </Card>
  );
