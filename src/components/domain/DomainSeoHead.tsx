...
export const DomainSeoHead: React.FC<Props> = ({ domain, analytics }) => {
  ...
  const sellerOrg: Record<string, unknown> = { "@type": "Organization", "name": sellerName, "url": SITE_ORIGIN,
  };
  // Optional locale/geo: platform is registered in CN — describe the marketplace org.
  const marketplaceOrg = { "@type": "Organization", "name": "域见•你", "url": SITE_ORIGIN, "logo": `${SITE_ORIGIN}/og-image.png`, "address": { "@type": "PostalAddress", "addressCountry": "CN", "addressLocality": "Global",
  ...
  const offer = { "@type": "Offer", "url": domainUrl, "priceCurrency": currency, "price": Number(domain.price || 0), "priceSpecification": { "@type": "PriceSpecification", "price": Number(domain.price || 0), "priceCurrency": currency, "valueAddedTaxIncluded": false,
    }, "availability": availability, "itemCondition": "https://schema.org/NewCondition", "priceValidUntil": (domain as any).price_valid_until || defaultPriceValidUntil(), "seller": sellerOrg, "offeredBy": marketplaceOrg, "areaServed": ["Worldwide"], "acceptedPaymentMethod": [ "https://schema.org/CreditCard", "http://purl.org/goodrelations/v1#PayPal",
    ], "eligibleRegion": { "@type": "Place", "name": "Worldwide" },
  };

  const structuredData: Record<string, unknown> = { "@context": "https://schema.org", "@type": "Product", "name": domain.name, "description": domain.description || `${domain.name} 域名出售`, "category": "Domain Name", "sku": domain.name, "brand": { "@type": "Brand", "name": "域见•你" }, "offers": offer,
  };
  if (domain.owner?.seller_rating) {
    structuredData.aggregateRating = { "@type": "AggregateRating", "ratingValue": domain.owner.seller_rating, "bestRating": "5", "worstRating": "1", "ratingCount": analytics?.offers || 1,
    };
  }
  ...
  const webPageData = { "@context": "https://schema.org", "@type": "WebPage", "name": title, "description": description, "url": domainUrl, "inLanguage": "zh-CN", "isPartOf": { "@type": "WebSite", "name": "域见•你", "url": SITE_ORIGIN }, "about": { "@type": "Thing", "name": domain.name, "description": `Domain name: ${domain.name}` }, "datePublished": domain.created_at, "interactionStatistic": [
