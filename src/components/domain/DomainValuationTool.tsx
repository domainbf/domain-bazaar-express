  ...
  const runEvaluation = async (domain: string) => {
    ...
    try {
      ...
      try {
        ...
        evalResult = { ...data, overallScore: data.overallScore ?? Math.round(Object.values(data.dimensions || {}).reduce((s: number, v: any) => s + (Number(v.score) || 0), 0) / 6), detectedFeatures: data.detectedFeatures ?? localEvaluate(d).detectedFeatures };
      } catch {
