        ...
        <div className="overflow-x-auto pb-4">
          <div className="flex gap-4 min-w-max">
            {soldDomains.map((domain) => (
              <div key={domain.id} className="flex-shrink-0 w-64 p-4 bg-card border border-border rounded-lg hover:shadow-md transition-all">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-semibold text-foreground truncate flex-1">{domain.name}</h3>
                  <CheckCircle className="w-4 h-4 text-primary flex-shrink-0 ml-2" />
                </div>
                <p className="text-sm text-muted-foreground">{t('soldDomains.soldFor', '成交价')}</p>
                <p className="text-lg font-bold text-primary">${domain.price.toLocaleString()}</p>
              </div>
            ))}
          </div>
        </div>
